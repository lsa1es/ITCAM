...
menu() {
	...
	case opcao in
	 "s") cat tmp/$p
		...
		echo "Voce Deseja executar: (s/n)"
		read opc
		case opc in
		"s")  $ISMCNF -config -change HTTP $Profile $index $Param=$Value
		;;
		"n") break
		;;
		esac	
					...
					do
						...
						if [ $qntd_pg -eq 1 ];then
							...
							echo "ismconfig.sh -config -change HTTP $Profile $Index $Param=$Value"
						        echo "Execute: "
							echo "$ISMCNF -config -change HTTP $Profile $index $Param=$Value"
							echo "Valide: "
							echo "$ISMCNF -config -listelts HTTP $Profile"
						fi
					done
				fi
			fi
